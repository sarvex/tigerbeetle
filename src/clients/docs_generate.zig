...
const samples = [_]Sample{
    ...
    .{
        ...
        \\transfer as posted. It then fetches both transfers, validates
    ...
    fn generate_sample_readmes(self: Generator, mw: *MarkdownWriter) !void {
        ...
        for (samples) |sample| {
            mw.print(
                ...
                \\[src/clients/docs_generate.zig](/src/clients/docs_generate.zig).
                \\
            , .{ language.proper_name, sample.proper_name });

            mw.print("# {s} Sample: {s}", .{ language.proper_name, sample.proper_name });
