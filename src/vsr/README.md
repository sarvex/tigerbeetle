...
If replica's `members_old` is not `null`, and it was an active replica in the past epoch, it continues processing messages in that epoch.

Interactions with other protocols:

Repairs can work across epochs.
Repair requests for headers and prepares from a past epoch can be directed at `members_old` replicas.

View changes do not happen in the past epoch.
Conceptually, `view` is saturated to ∞, any replica is primary, and any replica can send `start_view` with `.reconfiguration` as the head op.
